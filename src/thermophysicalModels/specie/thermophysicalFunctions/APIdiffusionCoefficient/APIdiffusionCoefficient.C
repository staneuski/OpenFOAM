...
{
    addScalarFunction2(APIdiffusionCoefficient);

    // Permit lookup with the old type name for backwards compatibility
    addNamedToRunTimeSelectionTable
    (
        scalarFunction2,
        APIdiffusionCoefficient,
        dictionary,
        APIdiffCoef
    );
}
...
:
    FieldFunction2<scalar, APIdiffusionCoefficient>(name),
    a_(a),
    b_(b),
    wf_(wf),
    wa_(wa),
